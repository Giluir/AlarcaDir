...
mod win {
    ...
    use windows::Win32::Storage::FileSystem::{
        CreateFileW, GetLogicalDriveStringsW, FILE_FLAG_NO_BUFFERING, FILE_FLAG_OPEN_REPARSE_POINT,
        FILE_FLAG_OVERLAPPED, FILE_READ_ATTRIBUTES, FILE_READ_DATA, FILE_SHARE_DELETE,
    ...
    pub fn get_ntfs_volume_data(volume_handle: HANDLE) -> Option<(u64, u64)> {
        ...
        if ok.is_ok() {
            Some((
                data.BytesPerCluster as u64,
                data.BytesPerFileRecordSegment as u64,
            ))
        } else {
            None
        }
    ...
    pub fn get_retrieval_pointers(mft_handle: HANDLE) -> Option<Vec<(u64, u64)>> {
        let mut buf: Vec<u8> =
            vec![0u8; std::mem::size_of::<RETRIEVAL_POINTERS_BUFFER>() + 128 * 16];
        ...
        let extents = unsafe { std::slice::from_raw_parts(rp.Extents.as_ptr(), count) };
...
fn process_mft_chunk(
    ...
    while off + record_size <= chunk.len() {
        ...
        for i in 1..usa_count {
            ...
            if sector_end_byte + 2 > record_size {
                break;
            }
        ...
        while attr_off + 8 <= record_size {
            ...
            if rec_len == 0 || attr_off + rec_len > record_size {
                break;
            }
            ...
            match type_code {
                ATTR_STANDARD_INFORMATION if !is_nr => {
                    let val_off =
                        unsafe { read_u16_at(attr_ptr, RESIDENT_VALUE_OFFSET_OFF) } as usize;
                    if attr_off + val_off + std::mem::size_of::<StandardInformation>()
                        <= record_size
                    {
                ...
                ATTR_DATA => {
                        ...
                        if name_len * 2 + name_off <= rec_len {
                            let stream_name_u16 = unsafe {
                                std::slice::from_raw_parts(
                                    attr_ptr.add(name_off) as *const u16,
                                    name_len,
                                )
                            };
                            ...
                            if sn == "WofCompressedData" && is_nr {
                                ...
                                if alloc > old_phys {
                                    counters
                                        .byte_count
                                        .fetch_add(alloc - old_phys, Ordering::Relaxed);
                                } else if old_phys > alloc {
                                    counters
                                        .byte_count
                                        .fetch_sub(old_phys - alloc, Ordering::Relaxed);
                                }
                            }
                        }
                        attr_off += rec_len;
                    ...
                    if is_nr {
                        ...
                        if new_phys > old_phys {
                            counters
                                .byte_count
                                .fetch_add(new_phys - old_phys, Ordering::Relaxed);
                        } else if old_phys > new_phys {
                            counters
                                .byte_count
                                .fetch_sub(old_phys - new_phys, Ordering::Relaxed);
                        }
                    } else {
                        let val_len =
                            unsafe { read_u32_at(attr_ptr, RESIDENT_VALUE_LENGTH_OFF) } as u64;
...
fn build_flat_nodes(
    ...
    while let Some((mft_idx, parent_node_id)) = queue.pop_front() {
        ...
        for child in children {
            if child.base_record < NTFS_RESERVED_MAX {
                continue;
            }
            ...
            counters
                .byte_count
                .fetch_add(rec.physical_size, Ordering::Relaxed);
            if !is_dir {
                counters.file_count.fetch_add(1, Ordering::Relaxed);
            }
...
pub fn scan(
    ...
    let (bytes_per_cluster, bytes_per_record) = win::get_ntfs_volume_data(vol_handle.0)?;
    ...
    data_runs
        .par_iter()
        .for_each(|&(lcn_start, cluster_count)| {
